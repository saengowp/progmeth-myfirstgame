package com.progmethgame.server.entities.bullets;

public final class BulletConfig {
	
	//Hook Bullet
	public static final float HOOK_BLLET_SPEED = 6.0f;
	public static final float HOOK_SPEED = 15.0f;

	//Teleport Bullet
	public static final float TELEPORT_BULLET_SPEED = 9.0f;
	
	//Burn Bullet
	public static final float BURN_BULLET_SPEED = 9.0f;
	
	//Confuse Bullet
	public static final float CONFUSE_BULLET_SPEED = 9.0f;
	
	//Slow Bullet
	public static final float SLOW_BULLET_SPEED = 9.0f;
	
	//Stunt Bullet
	public static final float STUNT_BULLET_SPEED = 9.0f;

}
